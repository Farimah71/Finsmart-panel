...
import { createCompanyBudgetFile } from "../../../../../redux/actions/settings/company-budget-file";
import { useDispatch } from "react-redux";
import { useSelector } from "react-redux";
import { Field } from "formik";
import * as Yup from "yup";

export const UploadBudgetFile = ({ headerId, onCloseModal, isReloadPage }) => {
  // -------- store ---------
  const { loading } = useSelector((state) => state.companyBudgetFileSlice);

  // -------- states -----------
  const [progress, setProgress] = useState(0);
  const [initialValue, setInitialValue] = useState({
    file: "",
    companyBudgetHeaderId: headerId,
  });
    ...
    <>
      <div className="flex justify-between">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white p-3">
          {t("page_title.upload_budget_file")}
        </h4>
        <div
