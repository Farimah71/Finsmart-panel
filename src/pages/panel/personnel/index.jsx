...
import { setLoading } from "../../../redux/reducers/settings/personnel";
import { CreatePersonnel } from "./create";
...
export const Personnel = () => {
  // --------- store --------
  const { info, loading } = useSelector((state) => state.personnelSlice);
  ...
  const cols = [
    ...
    {
      name: t("table.col.first_name"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.firstname}</div>
      ),
    },
    {
      name: t("table.col.family"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.surname}</div>
      ),
    },
    {
      name: t("table.col.salary"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.salary}</div>
      ),
    },
    {
      name: t("table.col.start_date"),
      selector: (row) => (
    ...
    {
      name: t("table.col.exit_date"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">
          {row.exitDate && row.exitDate.substring(0, 10)}
        </div>
      ),
    },
    {
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deletePersonnel + id)
      .then((res) => {
