...
import { getCodingFlags } from "../../../../redux/actions/settings/coding-flag";
import { Button, Spinner, Table } from "../../../../components";
import { CreateCodingFlag } from "./create";
import { EditCodingFlag } from "./edit";
import { api } from "../../../../api";
import {
  errorNotification,
  successNotification,
...
export const CodingFlag = ({ onCloseModal, state: id }) => {
  // ---------- states ----------
  ...
  const { info: flagData, loading } = useSelector(
    (state) => state.codingFlagSlice
  );
  ...
  const cols = [
    ...
    {
      name: t("table.col.keyword"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.keyword}</div>
      ),
    },
    {
      name: t("table.col.flag_type"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.flagType}</div>
      ),
    },
    {
  ...
  useEffect(() => {
    if (flagData) {
      const values = flagData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.codingFlagId,
        keyword: item.keyword,
        flagType: selectOptions.find((option) => option.value === item.flagType)
          .label,
      }));
      setData(values);
    }
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.SettingsApi.deleteCodingFlag + id)
      .then((res) => {
  ...
  return (
    <>
      <div className="dark:bg-dark_custom-light-black rounded-10">
        ...
        <hr />
        <div className="flex justify-between w-full p-4 bg-white dark:bg-dark_custom-light-black">
          ...
          <div className="flex justify-between items-center dark:bg-dark_custom-light-black">
            <Button
              ...
              classes="px-4"
              onClick={() => setSideAction({ type: "create" })}
            />
          </div>
        </div>
        <div
          className={`w-full px-6 bg-white dark:bg-dark_custom-light-black overflow-y-scroll ${
            flagData?.count > 2 ? "h-[165px]" : "h-auto"
          } `}
        >
          {isLoading ? <Spinner /> : <Table cols={cols} data={data} />}
        </div>
        <div className="bg-blue-50 rounded-11 m-3 mb-3 dark:border-none">
