...
import { setLoading } from "../../../redux/reducers/settings/customer";
...
export const Customer = () => {
  // --------- store --------
  const { info, loading } = useSelector((state) => state.customerSlice);
  ...
  const cols = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.name}</div>
      ),
    },
    {
    ...
    {
      name: t("table.col.city"),
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.city}</div>
      ),
    },
    {
  ...
  useEffect(() => {
    dispatch(
      getCustomers({
        ...
        includeProperties: "Country,City",
      })
    );
  }, [isReload]);
  useEffect(() => {
