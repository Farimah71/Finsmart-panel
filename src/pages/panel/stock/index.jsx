...
import { setLoading } from "../../../redux/reducers/settings/stock";
...
export const Stock = () => {
  // --------- store --------
  const { info, loading } = useSelector((state) => state.stockSlice);
  ...
  const cols = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div className="dark:text-dark_custom-full-white">{row.code}</div>
      ),
    },
    {
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteStock + id)
      .then((res) => {
          ...
          <div className="flex justify-between items-center dark:bg-dark_custom-light-black">
            <div className="flex flex-col gap-y-1">
              ...
              <h4 className="text-14 flex gap-x-2 text-custom-gray-muted dark:text-dark_custom-light-white">
                {t("text.dashboard")} - {t("page_title.stock")}
              </h4>
            </div>
            <Button
