...
import { CreateCodeMapper } from "./create";
import { EditCodeMapper } from "./edit";
import {
  errorNotification,
  successNotification,
...
export const CodeMapper = ({ onCloseModal, isReloadPage, id }) => {
  // ---------- states ----------
  const [data, setData] = useState();
  const [sideAction, setSideAction] = useState({ type: "", data: "" });
            ...
            <>
              {action.type === "edit" && (
                <span
                  ...
                  onClick={() => setSideAction({ type: "edit", data: row.id })}
                >
                  <svg
                    width="17"
                    height="17"
  ...
  useEffect(() => {
    const element = document.getElementById("mapper_forms");
    element.scrollIntoView({ behavior: "smooth" });
    const formTop = document.getElementById("form_top");
    !sideAction.type && formTop.scrollIntoView({ behavior: "smooth" });
  }, [sideAction.type]);
  ...
  const deleteHandler = (id) => {
    ...
    axios
      .delete(api.SettingsApi.deleteCodeMapper + id)
      .then((res) => {
  ...
  return (
    <>
      ...
      <div
        className="dark:text-dark_custom-full-white flex justify-between p-3 mb-1"
        id="form_top"
      >
        <h4 className="text-custom-dark font-bold dark:text-dark_custom-full-white mt-2">
          {t("page_title.code_mapper")}
        </h4>
      </div>
      <hr />
      <hr />
      {id ? (
        <div className="flex justify-between py-4 px-1 items-center dark:bg-dark_custom-light-black">
          ...
          <Button
            ...
            classes="px-4"
            onClick={() => {
              setSideAction({ type: "create" });
            }}
          />
        </div>
      ) : (
        <p className="mx-auto w-fit mt-5 text-purple-300">
          {t("text.click_coding")}
        </p>
      )}
      {loading && (
        <PulseLoader size={8} color="#95ebf5" className="text-center" />
      )}
      <div
        className={`w-full bg-white dark:bg-dark_custom-light-black overflow-y-scroll overflow-hidden ${
          sideAction.type && "h-[200px]"
        } `}
      >
        <Table cols={cols} data={data} />
      </div>
      <div className="dark:bg-dark_custom-light-black mb-3">
        <div className="px-2">
          <div
            className="bg-blue-50 dark:bg-transparent rounded-11 my-10 dark:border-none overflow-hidden"
            id="mapper_forms"
          >
            {sideAction.type === "create" && (
              <CreateCodeMapper
                id={id}
                onCloseModal={() => setSideAction({ type: "", data: "" })}
                isReloadPage={() =>
                  reloadPageHandler((currState) => !currState)
                }
              />
            )}
            {sideAction.type === "edit" && (
              <EditCodeMapper
                onCloseModal={() => setSideAction({ type: "", data: "" })}
                isReloadPage={() =>
                  reloadPageHandler((currState) => !currState)
                }
                editId={sideAction.data}
              />
            )}
          </div>
        </div>
